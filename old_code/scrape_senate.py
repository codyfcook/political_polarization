...
## MAIN ## 
# Find links
links = scrape_thomasLOC_links.scrape_all_links(2011, 1, 2012, 12, True)
pickle.dump(links, open("senate_links_dict.p", "wb"))

# Find all text on those links 
links = pickle.load(open("senate_links_dict.p", "rb"))
text_dict = scrape_thomasLOC.create_text_dict(links)
pickle.dump(text_dict, open("senate_text_dict.p", "wb"))

# Clean text dict 
text_dict = pickle.load(open("senate_text_dict.p", "rb"))
cleaned_text = process_thomasLOC_text.clean_text(text_dict)
pickle.dump(cleaned_text, open("senate_cleaned_text_dict.p", "wb"))

# Create speakers dictionary -- splitting by speaker within each day-topic 
cleaned_text = pickle.load(open("senate_cleaned_text_dict.p", "rb"))
...
pickle.dump(speakers_dict, open("senate_speakers_dict.p", "wb"))
